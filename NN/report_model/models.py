...
class DataSet():
    ...
    def __init__(self, model, name, data_dir, loss_function="mse"):
        self.model = model
        self.name = name
    ...
    def _evaluate(self, y_true, y_pred, loss_function=None):
        ...
        if loss_function == "cross_entropy":
            ...
            entropy = -np.mean(np.sum(y_true * np.log(y_pred), axis=1))
            return entropy + self.model.regularization()

        y_true_denormalized = self.y_scaler.inverse_transform(y_true)
        y_pred_denormalized = self.y_scaler.inverse_transform(y_pred)
        if loss_function == "mse":
            loss = np.mean((y_true_denormalized - y_pred_denormalized) ** 2)
        if loss_function == "mae":
            loss = np.mean(np.abs(y_true_denormalized - y_pred_denormalized))
        return loss + self.model.regularization()
    ...
    def plot_test(self):
        X_test = self.x_scaler.inverse_transform(self.X_test)
        y_test = self.y_scaler.inverse_transform(self.y_test)
        plt.scatter(X_test, y_test, color="blue", label="Test Data", s=5)
        plt.xlabel("Input")
        plt.ylabel("Output")
    ...
    def plot_train(self):
        X_train = self.x_scaler.inverse_transform(self.X_train)
        y_train = self.y_scaler.inverse_transform(self.y_train)
        plt.scatter(X_train, y_train, color="red", label="Training Data", s=5)
        plt.xlabel("Input")
        plt.ylabel("Output")
        plt.title(f"Dataset {self.name}")
        plt.legend()
...
class MLP():
    ...
    def __init__(
        ...
        self.regularization = self.dummy_regularization
    ...
    def l1_regularization(self):
        ...
        weight_count = 0
        for layer in self.layers:
            reg_loss += np.sum(np.abs(layer.weights))
            weight_count += layer.weights.size
        reg_loss /= weight_count
    ...
    def train(
        ...
        regularization=None,
        reg_lambda=1e-2,
        save_till_best=False
    ):

        if regularization == "l1":
            self.regularization = self.l1_regularization
        elif regularization == "l2":
            self.regularization = self.l2_regularization
        ...
        print(
            f"Model training finished at age {self.age} with loss {test_loss} on test set"
        )
    ...
    def plot_history(self, start_age=0, end_age=None, smoothing_interval=1, scale="log"):
        if end_age is None:
            end_age = self.age
    ...
    def plot_classification_data(self):
        plt.figure(figsize=(12, 6))
        plt.suptitle(f"Dataset: {self.data.name.capitalize()}", fontsize=16)
        plt.subplot(1, 2, 1)
