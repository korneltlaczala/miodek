...
class App:
    def __init__(self, trainer_name):
        ...
        self.trainer = Trainer.load(trainer_name)
        self.trainer_frame = TrainerFrame(self.root, self, self.trainer)
...
class TrainerFrame(ctk.CTkFrame):
    ...
    def __init__(self, parent, app, trainer, **kwargs):
    ...
    def update_save_frame(self):
        ...
        self.save_button = ctk.CTkButton(self.save_frame, text="Save Trainer", command=self.save_trainer)
        self.save_button.grid(row=1, column=0, padx=5, pady=5, sticky='nsew')
        self.restore_button = ctk.CTkButton(self.save_frame, text="Restore Trainer", command=self.restore_trainer)
    ...
    def fill_progress_frame(self):
        ...
        self.best_mse_label = ctk.CTkLabel(self.progress_frame, text="Best MSE: " + str(self.trainer.best_mse), anchor='w')
        self.best_mse_label.grid(row=2, column=2, padx=5, pady=5, sticky='nsew')

    def fill_settings_frame(self):
        self.learning_rate_label = ctk.CTkLabel(self.settings_frame, text=f"Learning Rate: {self.trainer.learning_rate}", anchor='w')
        ...
        for i, rate in enumerate(learning_rates):
            button = ctk.CTkButton(self.settings_frame, text=str(rate), 
                                   command=lambda r=rate: self.set_learning_rate(r))
            button.grid(row=1, column=i, padx=5, pady=5, sticky='nsew')
            self.learning_rate_buttons.append(button)

        self.new_epochs_label = ctk.CTkLabel(self.settings_frame, text=f"New Epochs: {self.trainer.new_epochs}", anchor='w')
        self.new_epochs_label.grid(row=2, column=0, padx=5, pady=5, sticky='nsew')

        self.epoch_multiplier_combobox = ctk.CTkComboBox(self.settings_frame, values=[str(i) for i in range(1, 11)], command=lambda v: self.set_epoch_multiplier())
        ...
        for i, choice in enumerate(epoch_choices):
            button = ctk.CTkButton(self.settings_frame, text=str(choice), 
                                   command=lambda c=choice: self.set_epoch_oom(c))
    ...
    def set_new_epochs(self):
        self.trainer.new_epochs = self.epoch_multiplier * self.epoch_oom
        self.new_epochs_label.configure(text=f"New Epochs: {self.trainer.new_epochs}")

    def fill_button_frame(self):
        self.start_button = ctk.CTkButton(self.button_frame, text="Start", command=self.start_training)
